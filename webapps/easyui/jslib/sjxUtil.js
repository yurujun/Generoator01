...
var sjx = $.extend({}, sjx);/*sjx定义全局对象，类似于命名空间或包的作用 */
...
$.extend($.fn.datagrid.defaults.editors, {
	combocheckboxtree : {
		...
		setValue : function(target, value) {
			$(target).combotree('setValues', sjx.getList(value));
		},
	...
	multiplecombobox : {
		...
		setValue : function(target, value) {
			$(target).combobox('setValues', sjx.getList(value));
		},
		resize : function(target, width) {
 ...
 *            主题名称
 */
sjx.changeTheme = function(themeName) {
 ...
 * 增加formatString功能
 * 
 * 使用方法：sjx.fs('字符串{0}字符串{1}字符串','第一个变量','第二个变量');
 * 
 * @returns 格式化后的字符串
 */
sjx.fs = function(str) {
 ...
 * 增加命名空间功能
 * 
 * 使用方法：sjx.ns('jQuery.bbb.ccc','jQuery.eee.fff');
 */
sjx.ns = function() {
	var o = {}, d;
 ...
 * @returns UUID字符串
 */
sjx.random4 = function() {
	return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1);
};
sjx.UUID = function() {
	return (sjx.random4() + sjx.random4() + "-" + sjx.random4() + "-" + sjx.random4() + "-" + sjx.random4() + "-" + sjx.random4() + sjx.random4() + sjx.random4());
};
 ...
 * @returns object
 */
sjx.serializeObject = function(form) {
	var o = {};
