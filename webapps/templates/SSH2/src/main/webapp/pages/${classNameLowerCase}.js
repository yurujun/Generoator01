...
${'$'}(function() {
	...
	datagrid = ${'$'}('#datagrid').datagrid({
		url : basePath + '/${classNameLower}Action!find.action',
		iconCls : 'icon-save',
		pagination : true,
		pagePosition : 'bottom',
		pageSize : 10,
		...
		onAfterEdit : function(rowIndex, rowData, changes) {
			...
			if (inserted.length > 0) {
				url = '${classNameLower}Action!add.action';
			}
			if (updated.length > 0) {
				url = '${classNameLower}Action!edit.action';
			}
			${'$'}.ajax({
				url : url,
				data : rowData,
	...
	if (rows.length > 0) {
		${'$'}.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
			if (r) {
				...
				${'$'}.ajax({
					url : '${classNameLower}Action!delete.action',
					data : {
						ids : ids.join(',')
					},
					dataType : 'json',
